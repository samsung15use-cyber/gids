...
# ========== ⚙️ НАСТРОЙКИ ==========
TOKEN = "8559247892:AAEYcK5RM9onRAS_COy0XzwW7gbWiinU6u4"
...
class AdminStates(StatesGroup):
    broadcast_msg = State()  # Рассылка пользователям
    broadcast_channels_msg = State()  # Рассылка в каналы
...
async def check_subscription(cb: CallbackQuery):
    ...
    if not not_subscribed:
        # Если подписан на все каналы - сразу показываем выбор подарка
        kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        # Если есть неподписанные каналы - показываем их в виде кнопок
        text = (
...
async def verify_subscription(cb: CallbackQuery):
    ...
    if not not_subscribed:
        # Если подписан на все каналы - показываем выбор подарка
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🖼 Забрать NFT", callback_data="finish")],
            [InlineKeyboardButton(text="⭐ Забрать 700 Звезд", callback_data="finish")]
        ])
        ...
        # Если остались неподписанные каналы - показываем их
        text = (
...
async def del_ch_list(cb: CallbackQuery):
    ...
    # Разбиваем кнопки на строки по 2 для удобства
    buttons = []
...
async def confirm_channels_broadcast(cb: CallbackQuery, state: FSMContext):
    ...
    report += "\n".join(results[:20])  # Показываем первые 20 результатов
